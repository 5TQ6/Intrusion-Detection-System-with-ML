...
<h3>Key Workflows: </h3>
  ...
  • Phase 1: Exhaustive Feature Selection Loop:
    ...
    ◦ Output: Automatically generates and saves subsetted datasets (CSVs) for every configuration into structured directories (e.g., data frames/main/RF/15 features , for random forest as the          evaluation model).
...
<h3>Key Workflows: </h3>
  - **Advanced Data Sampling Strategy:**
    - Implements the **"Hybrid-100k" Sampling Protocol**: A custom logic that combines **Random Undersampling** of majority classes (Benign, UDPFlood) to a 100,000-sample ceiling with                  **Oversampling** (SMOTE/Hybrid) for minority classes. (Note that you can change the sampling method to many choices or even remove the sampling)
...
- **Robust Data Cleaning (`clean_database`):**
    ...
    - **Skewness/Scaling**: Applies `PowerTransformer` (Yeo-Johnson) to normalize non-Gaussian traffic distributions if the **fix_skewness** is set to True, or apply a Standard scaler if **do_scale** is set to True.
